...
logger.setLevel(logging.DEBUG) #the logging level can be changed to display info of different importance

target_geo = vam.geometry.TargetGeometry(imagefilename=vam.resources.load("flower.png"),pixels=512, binarize_image=False)
...
attenuation_model = vam.medium.AttenuationModel(coord_vec, type = 'analytical', form='homogeneous_cylinder', R = 1, alpha_internal = 1e-3)
...
P_rt = vam.projector.pyTorchRayTrace.PyTorchRayTracingPropagator(target_geo, proj_geo_rt, output_torch_tensor=False)

P_matrix = P_rt.buildPropagationMatrix()
save_path = r'D:\archive\projects\LDCT\P_matrix for examples (360 angles over 360, 512x512).npz'
import scipy.sparse
...
proj_geo_al = vam.geometry.ProjectionGeometry(angles,
                                            ray_type='algebraic',
                                            CUDA=False,
                                            loading_path_for_matrix = save_path
                                            )



optimizer_params = vam.optimize.Options(method='BCLP',n_iter=30, filter='ram-lak',verbose='plot', p=2, learning_rate = 10, response_model = response_model, eps=0.1)

opt_sino, opt_recon, opt_response, logs = vam.optimize.optimize(target_geo, proj_geo_al, optimizer_params, output = 'full')
