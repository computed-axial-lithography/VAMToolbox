...
def buildMatrix(target_geo, proj_geo, sparse_format = 'csr' ,save_path = None, block_insert = False):
    """
    ...
    P = sparse.lil_matrix((n_matrix_row,n_matrix_col)) #propagation (projection) matrix. 
    ...
    print('Building sparse matrix with f{n_matrix_row} rows and f{n_matrix_col} columns.')
    ...
    if block_insert:
        ...
        for row in range(n_matrix_row):
            ...
            print(f'Completed row: {row} = ({row*100/n_matrix_row :2.1f}%) at {time.perf_counter()-start_time : .1f}s')
