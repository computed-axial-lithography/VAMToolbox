...
class PyTorchAlgebraicPropagator:
    ...
    def __init__(self, target_geo, proj_geo, output_torch_tensor = False, dtype = torch.float32):
        ...
        if (self.n_col_eff % self.n_col_internal) != 0:
            raise Exception(f'The imported sparse matrix has {self.n_col_internal} columns, and it cannot match or be tiled to match the total number of voxels ({n_col_eff}) in the real space target.')
