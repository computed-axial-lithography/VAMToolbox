...
class PyTorchRayTracingPropagator():
    ...
    def __init__(self, target_geo, proj_geo, output_torch_tensor = False) -> None:
        ...
        self.max_num_step = self.target_geo.nY*(2*1.5) #_R2R_   2 is the assumed (voxel size/step size) ratio, 1.5 is safety margin 
    ...
    def forward(self, f):
        ...
        # #transform f from unwrapped to wrapped domain _f
        # _f = self.transformation.unwrappedToWrapped(f)

        #perform ray tracing integration in wrapped domain
    ...
    def backward(self, ray_energy_0):
        ...
        # np.save(r"test_output\dose_coefficient_wrapped_rays.npy",np.flipud(deposition_grid[:,:,0].T.cpu().numpy()))
        # deposition_grid = self.target_geo.transform.wrappedToUnwrapped(deposition_grid)
        # np.save(r"test_output\dose_coefficient_unwrapped_rays.npy",deposition_grid[:,:,0].cpu().numpy())

        fig, axs = plt.subplots(1,1)
        axs.imshow(np.flipud(deposition_grid[:,:,0].T.cpu().numpy()),extent=[0,self.target_geo.nX/self.target_geo.transform.spatial_sampling_rate_rho,0,self.target_geo.nY/self.target_geo.transform.spatial_sampling_rate_rho],cmap='gray')
        axs.set_xlabel(r'$x$ [cm]')
        axs.set_ylabel(r'$y$ [cm]')
        # fig.savefig(r"rays_wrapped_n_ratio_.png",dpi=300)
        #transform wrapped to unwrapped domain
        deposition_grid = self.target_geo.transform.wrappedToUnwrapped(deposition_grid)
        
        fig, axs = plt.subplots(1,1)
        axs.imshow(deposition_grid[:,:,0].cpu().numpy(),extent=[0,self.target_geo.nL/self.target_geo.transform.spatial_sampling_rate_l,self.target_geo.transform.r_o,self.target_geo.transform.r_i],cmap='gray')
        axs.set_xlabel(r'$l$ [cm]')
        axs.set_ylabel(r'$\rho$ [cm]')
        # fig.savefig(r"rays_unwrapped_n_ratio_.png",dpi=300)
        plt.show()
    ...
    def buildPropagationMatrix(self):
        ...
        # Added for R2R #
        P_matrix_sino2uw_shape = (propagation_matrix_coo.shape[0],self.target_geo.transform.N_rho_uw*self.target_geo.transform.N_l_uw)
        P_matrix_sino2uw = torch.sparse_coo_tensor(size=P_matrix_sino2uw_shape, device=self.device)


        # transpose original propagation matrix for PSF extraction
        propagation_matrix_coo = propagation_matrix_coo.t()

        batch_size = 1
        n_batches = int(np.ceil(P_matrix_sino2uw_shape[0]/batch_size))
        for i in range(n_batches):
            _i_start = i*batch_size
            _i_end = (i+1)*batch_size
            if _i_end > P_matrix_sino2uw_shape[0]:
                _i_end = P_matrix_sino2uw_shape[0]
                internal_batch_size = _i_end - _i_start
            else:
                internal_batch_size = batch_size

            print(f'psf sinogram pixel {_i_end+1}/{propagation_matrix_coo.shape[1]}')

            # extract psf for each sinogram pixel in wrapped domain
            index_select = torch.arange(_i_start,_i_end,dtype=torch.long,device=self.device)
            psf_wrapped = propagation_matrix_coo.index_select(dim=1,index=index_select).to_dense()
            psf_wrapped = torch.reshape(psf_wrapped,shape=(self.solver.index_model.xg.shape[0],self.solver.index_model.xg.shape[1],internal_batch_size))
            # transform psf from wrapped domain to unwrapped domain
            psf_unwrapped = self.target_geo.transform.wrappedToUnwrapped(psf_wrapped)
            
                
            # psf_unwrapped = torch.reshape(psf_unwrapped,shape=(-1,))
            psf_unwrapped = psf_unwrapped.reshape((-1,))

            # add wrapped psf to final propagation matrix
            values_mask = psf_unwrapped.gt(0.0)



            rows = torch.zeros(size=(P_matrix_sino2uw_shape[1]*internal_batch_size,),device=self.device)
            cols = torch.zeros(size=(P_matrix_sino2uw_shape[1]*internal_batch_size,),device=self.device)
            for j in range(internal_batch_size):
                j_start = j*P_matrix_sino2uw_shape[1]
                j_end = (j+1)*P_matrix_sino2uw_shape[1]
                rows[j_start:j_end] = torch.full(size=(P_matrix_sino2uw_shape[1],),fill_value=_i_start+j,device=self.device)
                cols[j_start:j_end] = torch.arange(end=P_matrix_sino2uw_shape[1],device=self.device)

            indices = torch.vstack(
                (torch.masked_select(rows,values_mask),torch.masked_select(cols,values_mask))
            )

            P_matrix_sino2uw += torch.sparse_coo_tensor(indices=indices,values=torch.masked_select(psf_unwrapped,values_mask),size=P_matrix_sino2uw_shape,device=self.device,dtype=P_matrix_sino2uw.dtype)

        #Output as a torch sparse tensor
        if self.output_torch_tensor == True:
            return P_matrix_sino2uw
        else: #Convert output to class scipy.sparse.coo_array if needed
            indices = P_matrix_sino2uw.indices().cpu().numpy() 
            values = P_matrix_sino2uw.values().cpu().numpy()
            return scipy.sparse.coo_array((values, (indices[0,:], indices[1,:])), shape=(P_matrix_sino2uw.size(dim=0), P_matrix_sino2uw.size(dim=1)))
        ############################################################################################################
        ############################################################################################################
        ############################################################################################################
...
class RayTraceSolver():
    ...
    def expressPositionInArrayIndices(self, x):
        ...
        return (x-(-torch.abs(self.index_model.xv_yv_zv_min)))*(1/self.index_model.voxel_size) #Normalize position with voxel size so that the position is indicative of the tensor index
...
class RayState():
    ...
    def setupRays(self, ray_trace_ray_config, target_coord_vec_list, azimuthal_angles_deg, inclination_angle_deg = 0, ray_density = 1, ray_width_fun = None):
        ...
        elif self.ray_trace_ray_config == 'r2r': # _R2R_
            self.setupRaysR2R(target_coord_vec_list)
    ...
    @torch.inference_mode()
    def setupRaysR2R(self, target_coord_vec_list):
        #The number of rays is assumed to be proportional to the size of real space grid/array
        real_nX = target_coord_vec_list[0].size 
        real_nY = target_coord_vec_list[1].size 
        real_nZ = target_coord_vec_list[2].size
        
        self.sino_shape_rd1 = (real_nX, self.azimuthal_angles_rad.size, real_nZ) #This is the desired shape when ray density = 1. Saved for binning purpose.
        
        if real_nZ == 1: #2D case
            self.sino_shape = (round(real_nX * self.ray_density), self.azimuthal_angles_rad.size, real_nZ) #no need to supersample or undersample in z
        else: #3D case
            self.sino_shape = (round(real_nX * self.ray_density), self.azimuthal_angles_rad.size, max(round(real_nZ * self.ray_density),1)) #At least compute 1 z layer
        self.num_rays = self.sino_shape[0]*self.sino_shape[1]*self.sino_shape[2]

        #The following assumes the patterning volume is inscribed inside the simulation cube
        #===========Coordinates local to projection plane===================
        #Determine first coordinate of sinogram
        sino_n0_min = np.amin(target_coord_vec_list[0])
        sino_n0_max = np.amax(target_coord_vec_list[0])
        sino_n0 = np.linspace(sino_n0_min, sino_n0_max, self.sino_shape[0])
        ...
        # this assumes that the ray traverses along the y axis so the radial offset should be such that the proj plane lies on the minimum bound of the y axis
        proj_plane_radial_offset = np.abs(np.amin(target_coord_vec_list[1]))
        
        #Create with GPU (which is faster if GPU memory is sufficient)
        sino_n0 = torch.as_tensor(sino_n0, device = self.device, dtype = self.tensor_dtype)
        sino_n1_rad = torch.as_tensor(sino_n1_rad, device = self.device, dtype = self.tensor_dtype)
        sino_n2 = torch.as_tensor(sino_n2, device = self.device, dtype = self.tensor_dtype)
        proj_plane_radial_offset = torch.as_tensor(proj_plane_radial_offset, device = self.device, dtype = self.tensor_dtype)
        G0, G1, G2 = torch.meshgrid(sino_n0, sino_n1_rad, sino_n2, indexing = 'ij')
        G0 = torch.ravel(G0)
        G1 = torch.ravel(G1)
        G2 = torch.ravel(G2)
        ...
        self.x_0 = torch.empty((self.num_rays,3), device = self.device, dtype = self.tensor_dtype) #using same date type as sino_n0, which is inferred from its numpy version
        #Center of projection plane relative to grid center. Refers to documentations for derivation.
        self.x_0[:,0] = proj_plane_radial_offset*torch.cos(G1)
        self.x_0[:,1] = proj_plane_radial_offset*torch.sin(G1)
        self.x_0[:,2] = 0

        #Adding vectors from center of projection plane to pixel. Refers to documentations for derivation.
        self.x_0[:,0]+= -G0*torch.sin(G1) 
        self.x_0[:,1]+= G0*torch.cos(G1)
        self.x_0[:,2]+= G2

        self.v_0 = torch.empty((self.num_rays,3), device = self.device, dtype = self.tensor_dtype)
        self.v_0[:,0] = -torch.cos(G1)
        self.v_0[:,1] = -torch.sin(G1)
        self.v_0[:,2] = 0 
