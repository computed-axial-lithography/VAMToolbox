...
class tigre3D():
    def __init__(self, target, params):
        self.target = target
        try:
            self.attenuation = np.swapaxes(params['attenuation'].astype(np.float32),2,0)
        ...
        self.geo = tigre.geometry(mode='parallel', nVoxel=np.array([self.nZ,self.nY,self.nX]))
    ...
    def forwardProject(self,target):
        ...
        projections = tigre.Ax(target,self.geo,self.angles_rad,projection_type='interpolated',img_att=self.attenuation)
    ...
    def backProject(self,projections):
        ...
        if self.attenuation is not None:
            ...
            reconstruction = tigre.Atb(projections,self.geo,self.angles_rad,img_att=tmp_attenuation)
        else:
...
if __name__ == "__main__":
    ...
    params = {
        'angles' : np.linspace(0,360-(360/180),100),
        'attenuation' : occl
    }
