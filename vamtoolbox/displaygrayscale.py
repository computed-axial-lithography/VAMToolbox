...
class IndexTracker():
	def __init__(self,array,vol_type,slice_index,label,slice_step,ax=None,fig=None,**kwargs):
		...
		if slice_index == 0:
			self.ind = self.n0//2
			self.im = self.ax.imshow(self.array[self.ind, :, :],vmin=0,vmax=v_max,**kwargs)
		elif slice_index == 1:
			self.ind = self.n1//2
			self.im = self.ax.imshow(self.array[:, self.ind, :],vmin=0,vmax=v_max,**kwargs)
		elif slice_index == 2:
	...
	def update(self):
		...
		if self.slice_index == 1 and (self.vol_type == 'recon' or self.vol_type == 'target'):
...
class VolumeSlicer():
	def __init__(self,array,vol_type,slice_step=1,show_bodies=False,ax=None,fig=None,**kwargs):
		"""
		Parameters
		----------
		...
		for ax,slice_ind,label in zip(self.axs,self.slice_inds,labels):
			if show_bodies == True:
				self.scroll_trackers.append(BodiesIndexTracker(self.array,vol_type=self.vol_type,slice_index=slice_ind,label=label,slice_step=slice_step,ax=ax,fig=self.fig,**kwargs))
			else:
				self.scroll_trackers.append(IndexTracker(self.array,vol_type=self.vol_type,slice_index=slice_ind,label=label,slice_step=slice_step,ax=ax,fig=self.fig,**kwargs))
...
class EvolvingPlot():
	...
	def update(self,error,dose=None, mapped=None):
