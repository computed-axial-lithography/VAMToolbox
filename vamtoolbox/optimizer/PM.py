...
def minimizePM(target_geo, proj_geo, _options):
    ...
    def loss(b_iter):
        ...
        if _options.bit_depth is not None:
            b_iter = vamtoolbox.util.data.discretize(
                b_iter, _options.bit_depth, [0.0, np.max(b_iter)]
            )
        ...
        loss_integrand = (
            _options.rho_1
            * np.power(dose_3D_iter - _options.d_l, _options.p)
            * V2_loc_iter
            + _options.rho_2
            * np.power(_options.d_h - dose_3D_iter, _options.p)
            * V1_loc_iter
        )
    ...
    def lossGradient(b_iter):
        ...
        grad_dose_iter = _options.rho_1 * np.double(
            V2_loc_iter
        ) * _options.p * np.power(
            dose_3D_iter - _options.d_l, _options.p - 1
        ) - _options.rho_2 * np.double(
            V1_loc_iter
        ) * _options.p * np.power(
    ...
    def callback(x):
        ...
        if _options.verbose == "time" or _options.verbose == "plot":
            print(
                "Iteration %4.0f at time: %6.1f s"
                % (
                    log_error.curr_iter + 1,
                    log_error.iter_times[log_error.curr_iter + 1],
                )
            )
    ...
    if b0.ndim == 3:
        bounds = [
            [[(bl, bh) for k in range(b0_shape[2])] for j in range(b0_shape[1])]
    ...
    return (
        vamtoolbox.geometry.Sinogram(b_opt, proj_geo, _options),
        vamtoolbox.geometry.Reconstruction(x_opt, proj_geo, _options),
        log_error.error,
    )
