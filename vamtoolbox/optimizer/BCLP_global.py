...
def minimizeBCLP(target_geo,proj_geo,_options):
    """
    ...
    """
    #Cost function on publication is internally renamed as loss function to be consistent with rest of VAMToolbox.
    
    def getRegions(mapped_dose_error):
        #get constraint violation regions
        #V = np.zeros_like(mapped_dose_error)
        V = (np.abs(mapped_dose_error) > _options.eps) #eps has to be a scalar or array of the same size as mapped_dose_error
    ...
    def loss(b_iter):
        ...
        mapped_dose = response_model.map(dose_3D_iter)

        global mapped_dose_error
        mapped_dose_error = mapped_dose - _options
        V = getRegions(mapped_dose_error)
        ...
        loss_integrand = V*_options.weight*
    ...
    def callback(x):
        ...
        #TODO:Compute other metrics and store them in log_error 
        log_error.error_iter = vamtoolbox.metrics.calcVER(target_geo,mapped_dose)
    ...
    global response_model
    response_model = _options.response_model
