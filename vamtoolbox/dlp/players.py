...
class _Process(pyglet.window.Window):
    def __init__(self,*args,**kwargs):
        ...
        self.image_config = None if 'image_config' not in kwargs else kwargs['image_config']
    ...
    def run(self):
        ...
        if self.video is not None:
            ...
            width, height = self.video_player.format.width,self.video_player.format.height
            ...
            if self.sinogram is not None:
                ...
                if isinstance(self.sinogram,vamtoolbox.geometry.Sinogram):
                    self.image_seq = vamtoolbox.imagesequence.ImageSeq(self.image_config,sinogram=self.sinogram)
                else:
                    Exception("sinogram is not of type vamtoolbox.geometry.Sinogram")                      

                sprites = list()
                for k, image in enumerate(self.image_seq.images):
                    print("Loading image %s/%s"%(str(k).zfill(4),str(len(self.image_seq.images)-1).zfill(4)))
                ...
                assert isinstance(self.image_seq,vamtoolbox.imagesequence.ImageSeq), "image_seq must be of type imagesequence.ImageSeq"
            ...
            self.sequence = pyglet.image.Animation.from_image_sequence(sprites, dt, True)
