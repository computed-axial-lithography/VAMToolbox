...
class SetupWindow(pyglet.window.Window):
    def __init__(self,**kwargs):
        ...
        self.screen_orientation = "horizontal" if 'screen_orientation' not in kwargs else kwargs['screen_orientation']
        ...
        if self.windowed == True:
            ...
            assert self.N_screen != None, "Screen size (N_screen) must be specified in windowed mode. N_screen = (N_U,N_V)"
            self.N_U, self.N_V = self.N_screen
            super().__init__(self.N_U,self.N_V,visible=False,screen=screens[self.screen_num])
            # self.set_fullscreen(screen=screens[self.screen_num])
        else:
            self.N_U, self.N_V = selected_screen.width, selected_screen.height
            super().__init__(self.N_U,self.N_V,visible=False,style=pyglet.window.Window.WINDOW_STYLE_BORDERLESS,screen=screens[self.screen_num])
            self.set_fullscreen(screen=screens[self.screen_num])
...
class AxisAlignment(SetupWindow):
    def __init__(self,half_line_thickness=10,half_line_separation=200,center_offset=0,**kwargs):
        """
        Parameters
        ----------
    ...
    def update(self,_):
        ...
        print("half line thickness = %d, half line separation (center to center) = %d, center offset = %d\n"%(self.half_line_thickness,self.half_line_separation,self.center_offset))
    ...
    def constructLines(self):
        ...
        if self.screen_orientation == "horizontal":
            self.array[:,self.N_U//2-self.half_line_thickness+self.center_offset:self.N_U//2+self.half_line_thickness+self.center_offset] = 255
            self.array[:,self.N_U//2-self.half_line_separation-self.half_line_thickness+self.center_offset:self.N_U//2-self.half_line_separation+self.half_line_thickness+self.center_offset] = 255
            self.array[:,self.N_U//2+self.half_line_separation-self.half_line_thickness+self.center_offset:self.N_U//2+self.half_line_separation+self.half_line_thickness+self.center_offset] = 255
        elif self.screen_orientation == "vertical":
            self.array[self.N_V//2-self.half_line_thickness+self.center_offset:self.N_V//2+self.half_line_thickness+self.center_offset,:] = 255
            self.array[self.N_V//2-self.half_line_separation-self.half_line_thickness+self.center_offset:self.N_V//2-self.half_line_separation+self.half_line_thickness+self.center_offset,:] = 255
            self.array[self.N_V//2+self.half_line_separation-self.half_line_thickness+self.center_offset:self.N_V//2+self.half_line_separation+self.half_line_thickness+self.center_offset,:] = 255
...
def _createGrid(shape,offset=None):
    ...
    U,V = np.meshgrid(np.linspace(-1*aspect_ratio,1*aspect_ratio,dim_u),np.linspace(-1,1,dim_v))
    V = V - v_offset
    V = V - u_offset
...
if __name__ == '__main__':
    ...
    Focus(slices=16,windowed=True,screen_orientation="horizontal",N_screen=(1000,1000))