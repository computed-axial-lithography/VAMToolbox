...
def minimizeCAL(target_geo, proj_geo, options):
    ...
    def thresholdReconstruction(reconstruction):
        ...
        for i in range(0, len(test_thresholds)):
            ...
            if target_geo.n_dim == 2:
                total_gel_not_in_target = np.sum(
                    thresholded[target_geo.void_inds[0], target_geo.void_inds[1]]
                )
                total_gel_in_target = np.sum(
                    thresholded[target_geo.gel_inds[0], target_geo.gel_inds[1]]
                )
            else:
                total_gel_not_in_target = np.sum(
                    thresholded[
                        target_geo.void_inds[0],
                        target_geo.void_inds[1],
                        target_geo.void_inds[2],
                    ]
                )
                total_gel_in_target = np.sum(
                    thresholded[
                        target_geo.gel_inds[0],
                        target_geo.gel_inds[1],
                        target_geo.gel_inds[2],
                    ]
                )

            score[i] = total_gel_in_target / len(
                target_geo.gel_inds[0]
            ) - total_gel_not_in_target / len(target_geo.void_inds[0])
        ...
        thresholded_reconstruction = 1 / (
            1 + np.exp(-(float(options.sigmoid) * recon_sub_mu))
        )
    ...
    for curr_iter in range(options.n_iter):
        ...
        grad = delta_b * (1 - options.momentum) + delta_b_prev * options.momentum / (
            1 - options.momentum ** (curr_iter + 1)
        )  # apply momentum weighting on gradient
        b = (
            b - options.learning_rate * grad
        )  # Update b (take a step in the direction of the negative gradient)
        b = np.clip(b, a_min=0, a_max=None)
    ...
    b_opt[b_opt < 0] = (
        0  # final positivity constraint applied in case there are any negatives
    )
    ...
    return (
        ...
        vamtoolbox.geometry.Reconstruction(x_opt, proj_geo, options),
        _error,
    )
