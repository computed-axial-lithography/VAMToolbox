...
class gauss2D:
    def __init__(self, target, proj_params, optical_params):
        ...
        self.w_psf = (
            1.62
            * optical_params["wavelength"]
            * optical_params["n"]
            / (np.pi * optical_params["NA"] * np.sqrt(2 * np.log(2)))
        )
        self.w0 = np.sqrt(
            self.w_psf**2 + (self.w_pixel**2) * (optical_params["magnification"] ** 2)
        )
    ...
    def createGaussKernel(self):
        ...
        t = np.linspace(
            -self.nT * self.voxel_size / 2, self.nT * self.voxel_size / 2, self.nT
        )
        prop = np.linspace(
        ...
        im = ax.imshow(
            np.where(kernel > 1e-20, kernel, 1e-20),
            cmap="CMRmap",
            extent=[t.min() * 1e6, t.max() * 1e6, prop.min() * 1e6, prop.max() * 1e6],
    ...
    def setupInterpCoords(self, dimension, angle=None):
        ...
        for curr_angle in np.nditer(self.angles_rad):
            ...
            coords = [
                (c - lo) * (n - 1) / (hi - lo)
                for (lo, hi), c, n in zip(limits, [propi, ti], self.target.shape)
            ]
    ...
    def convolveWithGaussKernelFP(self, target):
        ...
        convolved_target = np.real(
            np.fft.ifftshift(
                mkl_fft.ifft(np.multiply(FFT_target, self.FFT_kernel)), axes=1
            )
        )
    ...
    def convolveWithGaussKernelBP(self, projection):
        ...
        convolved_backproj = np.real(
            np.fft.ifftshift(
                mkl_fft.ifft(
                    np.multiply(FFT_projection[np.newaxis, :], self.FFT_kernel)
                ),
                axes=1,
            )
        )
    ...
    def gaussFP(self, target):
        ...
        for curr_angle_ind in range(len(self.angles_rad)):
            ...
            interpolated = cupyx.scipy.ndimage.map_coordinates(
                targetcp, self.coords[curr_angle_ind], order=1, mode="constant", cval=0
            )
    ...
    def gaussBP(self, projections):
        ...
        for curr_angle_ind in range(len(self.angles_rad)):
            ...
            curr_backproj = self.convolveWithGaussKernelBP(
                projections[:, curr_angle_ind]
            )

            # interpolated = scipy.ndimage.map_coordinates(curr_backproj,coords,order=1,mode='constant',cval=0)
            # reconstruction += np.reshape(interpolated,reconstruction.shape)
            interpolated = cupyx.scipy.ndimage.map_coordinates(
                cp.asarray(curr_backproj),
                self.coords[curr_angle_ind],
                order=1,
                mode="constant",
                cval=0,
            )
...
if __name__ == "__main__":
    ...
    font = {"family": "sans-serif", "sans-serif": "arial", "weight": "bold", "size": 36}
    ...
    plot_extents = (
        np.array([-target.shape[0], target.shape[0], -target.shape[0], target.shape[0]])
        * voxel_size
        / 2
        * 1e6
    )
