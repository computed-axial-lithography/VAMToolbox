...
class gauss3D:
    ...
    def createGaussKernel(self):
        ...
        z = np.linspace(
            -self.nZ * self.voxel_size / 2, self.nZ * self.voxel_size / 2, self.nZ
        )
    ...
    def setupInterpCoords(self, dimension, angle=None):
        ...
        propi_coor = cp.ravel(
            (self.nProp - 1) * (prop - limits[0][0]) / (limits[0][1] - limits[0][0])
        )
        ti_coor = cp.ravel(
            (self.nT - 1) * (t - limits[1][0]) / (limits[1][1] - limits[1][0])
        )
        ZZi_coor = cp.ravel(
            (self.nZ - 1) * (ZZ - limits[2][0]) / (limits[2][1] - limits[2][0])
        )
    ...
    def convolveWithGaussKernelFP(self, target):
        ...
        convolved_target = cp.real(
            cp.fft.ifftshift(
                cp.fft.ifft2(cp.multiply(FFT_target, self.FFT_kernel), axes=(1, 2))
            )
        )
    ...
    def convolveWithGaussKernelBP(self, projection):
        ...
        convolved_backproj = cp.real(
            cp.fft.ifftshift(
                cp.fft.ifft2(
                    cp.multiply(FFT_projection[cp.newaxis, :, :], self.FFT_kernel)
                ),
                axes=(1, 2),
            )
        )
    ...
    def gaussFP(self, target):
        ...
        projections = cp.zeros(
            (self.nT, len(self.angles_rad), self.nZ), dtype=cp.float32
        )
        ...
        for curr_angle_ind in range(len(self.angles_rad)):
            ...
            interpolated = cupyx.scipy.ndimage.map_coordinates(
                targetcp, coordscp, order=1, mode="constant", cval=0
            )
    ...
    def gaussBP(self, projections):
        ...
        for curr_angle_ind in range(len(self.angles_rad)):
            ...
            # reconstruction += np.reshape(interpolated,reconstruction.shape)
            interpolated = cupyx.scipy.ndimage.map_coordinates(
                curr_backproj, coords, order=1, mode="constant", cval=0
            )
